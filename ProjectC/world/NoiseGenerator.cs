...
namespace ProjectC.world
{
    public class NoiseGenerator
    {
        ...
        public static float GenWormNoise(float x, float y, float range)
        {
            return GenFloatNoise(x * y, range);
        }

        public static int GenIntNoise(float xx, float range)
        {
            return (int)GenFloatNoise(xx, range);
        }

        public static float GenFloatNoise(float xx, float range)
        {
            xx /= 16;
            float noise = 0;
        ...
        {
            var rand = new Random(Dimension.Seed + (int) index);
            return (float)rand.NextDouble() * range;
        }
    }
}
 
namespace Minion2D.Helpers
{
